...
public:
    double minAreaFreeRect(vector<vector<int>>& points) { //1 <= points.length <= 50
        ...
        int n = points.size();
        long long ret = 0;
        unordered_map<int, unordered_map<long long, vector<vector<int>>>> mp; //key: distance between 2 points.
        if(n<=3)
            return 0.0;
        for(int i=0; i<n; i++) {
            int x1 = points[i][0];
    ...
    long long d2(int x1, int y1, int x2, int y2) {
