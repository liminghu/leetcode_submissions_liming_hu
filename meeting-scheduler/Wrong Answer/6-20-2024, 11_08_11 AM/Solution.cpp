    ...
    vector<int> minAvailableDuration(vector<vector<int>>& slots1, vector<vector<int>>& slots2, int duration) {
        ...
        while(i<sz1 && j<sz2) {
            if(slots1[i][1] < slots2[j][0]+duration) {
                ...
                if(slots1[i][1] - slots2[j][0]>=duration && slots1[i][1]-slots1[i][0]>duration && slots2[j][1]-slots2[j][0]>duration)  {
                    int start = slots2[j][0];
                    int end = slots2[j][0]+duration;
                    return {start, end};
                };
                if(slots2[j][1] - slots1[i][0]>=duration && slots1[i][1]-slots1[i][0]>duration && slots2[j][1]-slots2[j][0]>duration) {
