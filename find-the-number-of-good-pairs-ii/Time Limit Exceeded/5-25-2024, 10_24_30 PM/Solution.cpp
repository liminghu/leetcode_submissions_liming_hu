            ...
            if(new_nums1[i] == 0) {
             ...
             for(int j=0; j<new_nums2.size(); j++) {
                if(new_nums1[i] < new_nums2[j] || new_nums2[j] == 0)
                 ...
                 if(dict.find(new_nums1[i]*(long long)1000000+new_nums2[j])!=dict.end()) {
                    if( dict[new_nums1[i]*(long long)1000000+new_nums2[j]] == 1) {
                        ans++;
                        continue;
                    } else {
                        continue;
                 ...
                 if(  mod_val == 0) {
                    ans++;
                    dict[new_nums1[i]*(long long)1000000+new_nums2[j]] = 1;  //true
                 } else {
                    dict[new_nums1[i]*(long long )1000000+new_nums2[j]] = 0;   //false
                 }
             }            
        }
        return ans;
