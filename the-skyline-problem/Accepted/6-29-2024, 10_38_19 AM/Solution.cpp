...
public:
    vector<vector<int>> getSkyline(vector<vector<int>>& buildings) { //1 <= buildings.length <= 104
        ...
        vector<pair<int, int>> points;  //{x, height}
        ...
        //it will sort first by x, then height. same x, height will be sorted increasingly. 
        //so the pints will be always entering first.
        sort(points.begin(), points.end());
        
        int ongoingHeight = 0; //previous height.
        
        // points.first = x coordinate, points.second = height (negative, if entering)
        for(int i = 0; i < points.size(); i++){
            ...
            // after inserting/removing heightAtCurrentPoint, if there's a change
            auto pqTop = *pq.rbegin(); //end. the highest.
            if(ongoingHeight != pqTop){ //pgTop is always the highest.
                ongoingHeight = pqTop;
                ans.push_back({currentPoint, ongoingHeight}); //{x, height}
            }
        }
