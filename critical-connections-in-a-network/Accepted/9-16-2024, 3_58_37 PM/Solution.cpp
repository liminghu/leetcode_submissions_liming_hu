...
/*
Time Complexity: O(V+E) where V represents the number of vertices and E represents the number 
of edges in the graph. We process each node exactly once thanks to the rank dictionary also 
acting as a "visited" safeguard at the top of the dfs function. Since the problem statement 
mentions that the graph is connected, that means E>=V and hence, the overall time complexity 
would be dominated by the number of edges i.e. O(E).
*/
    int dfs(int node, int discoveryRank) { //find the loop.
        ...
        for (auto neighbor : graph_[node]) {//We iterate over all the neighbors of the node     
            if(rank_.find(neighbor) != rank_.end() && rank_[neighbor] == discoveryRank - 1) //Skip the parent.
                continue;           
            int recursiveRank = dfs(neighbor, discoveryRank + 1); // Recurse on the neighbor.
            
            // Step 1, check if this edge needs to be discarded.
            if (recursiveRank <= discoveryRank) { //there is a loop: this edge can be discarded.
