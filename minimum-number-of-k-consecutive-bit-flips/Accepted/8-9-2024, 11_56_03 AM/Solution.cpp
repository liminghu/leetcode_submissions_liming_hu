...
/*        int sz = nums.size();
       ...
       int sz = nums.size();
        ...
        for(int i=0; i<sz; i++) {
            if( i-k >= 0 && nums[i-k] == 2) //flipped before.
                cur_window_flips +=1;
            
            if((nums[i] + cur_window_flips) %2 == 0) { //q.size(): num of flips related to nums[i]. (nums[i] + q.size()) %2: current value of nums[i] after flips before i.
                if(i+k > sz)
                    return -1;
                res++;
                cur_window_flips += 1;
