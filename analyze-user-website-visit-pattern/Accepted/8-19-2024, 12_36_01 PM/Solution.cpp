...
public:
    vector<string> mostVisitedPattern(vector<string>& user, vector<int>& t, vector<string>& site) {
        ...
        */
            ...
            unordered_map<string, map<int, string>> mp; //user, <time, site>
            unordered_map<string, int> cnt; //sites tuple.
            int m_cnt = 0; //max count.
            string res;

            for (auto i = 0; i < user.size(); ++i) 
                mp[user[i]][t[i]] = site[i];

            for (auto u : mp) { //for each user.
                unordered_set<string> ts;
                for (auto it = begin(u.second); it != end(u.second); ++it) //u.second: map<time, site>, it is sorted.
