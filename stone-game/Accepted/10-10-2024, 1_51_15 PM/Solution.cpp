...
public:
    ...
    bool stoneGame(vector<int>& piles) { //There are an even number of piles arranged in a row, and each pile has a positive integer number of stones piles[i].
        int n = piles.size();
        vector<vector<int>> score = vector(n, vector<int>(n, 0));
        for(int i=0; i<n; i++)
            score[i][i] = piles[i];
    ...
    /*    int n = piles.size();
        memo_ = vector<vector<int>>(n, vector<int>(n, INT_MIN));
        return score(piles, 0, n-1) > 0;*/
    }
    /*int score(vector<int>& piles, int l, int r) {
        if(l==r)    
            return piles[l];
            if(memo_[l][r]==INT_MIN)
                memo_[l][r] = max( piles[l] - score(piles, l+1, r), piles[r] - score(piles, l, r-1) );
            return memo_[l][r];
    }*/
};