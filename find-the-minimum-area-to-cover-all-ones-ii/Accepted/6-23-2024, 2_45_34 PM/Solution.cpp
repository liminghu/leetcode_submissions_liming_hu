...
unordered_map<string, int> area; 
    ...
    int minimumArea(vector<vector<int>>& grid, int left, int right, int top, int bottom) {
        ...
        int end_col = -1;
        int found = 0;
        string key = to_string(left)+ "_"+to_string(right)+ "_"+to_string(top)+ "_"+to_string(bottom);
        if(area.find(key)!=area.end())
            return area[key];
